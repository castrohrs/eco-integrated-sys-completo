...
const MaintenanceModal: React.FC<{
    ...
    React.useEffect(() => {
        if (taskToEdit) {
            ...
            setCost(0);
            setNotes('');
            setStatus('Concluída'); // Default to completed as we removed scheduling view
        }
    ...
    return (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[1001]" onClick={onClose}>
            <div className="bg-bg-card p-8 rounded-lg shadow-xl w-full max-w-lg text-light" onClick={e => e.stopPropagation()}>
                <form onSubmit={handleSubmit}>
                    <h2 className="text-2xl font-bold mb-6">{taskToEdit ? 'Editar Manutenção' : 'Registrar Manutenção'}</h2>
                    <div className="space-y-4">
                        ...
                        <div className="form-group"><label>Status</label><select value={status} onChange={e => setStatus(e.target.value as MaintenanceStatus)} className="input-style"><option>Concluída</option><option>Agendada</option></select></div>
                    </div>
                     <div className="mt-6 flex justify-end gap-4">
                        <button type="button" onClick={onClose} className="btn-style bg-border-color hover:bg-opacity-80">Cancelar</button>
                        <button type="submit" className="btn-style bg-primary hover:bg-opacity-90">Salvar</button>
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h1 className="text-2xl font-bold text-light">{t('fleetControl')}</h1>
                <button onClick={() => window.print()} className="px-4 py-2 bg-bg-card border border-border-color text-gray-400 hover:text-white rounded-lg text-xs font-bold uppercase transition-all flex items-center gap-2">
            ...
            <div className="bg-bg-card rounded-lg p-5 shadow-lg">
                <div className="flex justify-between items-center mb-4">
                    <h3 className="text-lg font-semibold text-light">Histórico de Manutenções Realizadas</h3>
                    <button onClick={() => setMaintModalOpen(true)} className="px-4 py-2 bg-primary text-white font-semibold rounded-md hover:bg-opacity-90 flex items-center gap-2 shadow-md hover:shadow-lg transition-shadow" disabled={!fleetData || fleetData.length === 0} title={(!fleetData || fleetData.length === 0) ? 'Adicione um veículo primeiro' : ''}><i className="fas fa-tools"></i> Registrar Manutenção</button>
                </div>
                 <div className="overflow-x-auto max-h-96">
                    <table className="w-full text-sm text-left text-gray-text">
                        <thead className="text-xs text-gray-400 uppercase bg-gray-700 sticky top-0">
